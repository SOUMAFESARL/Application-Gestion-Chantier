...
/**
 * Primitive `calendar` de shadcn (`npx shadcn@latest add calendar`).
 *
 * Trois écarts au fichier généré :
 *
 * 1. L'import de `cn` pointe vers `@/lib/utils` (voir `popover.tsx`).
 * 2. **La locale est celle de l'application** (`i18n/langue.ts`), passée par
 *    défaut : sans elle, le calendrier s'affiche en anglais et commence la
 *    semaine un dimanche — ce qu'aucun planning de chantier ne fait.
...
function Calendar({
  ...
  // Hors du JSX : ce sont des classes, pas du texte, mais la règle i18n ne
  // connaît que l'attribut `className` et lirait `classNames` comme une phrase.
  const classes: React.ComponentProps<typeof DayPicker>["classNames"] = {
    ...
    caption_label: cn(
      "font-medium select-none first-letter:uppercase",
      captionLayout === "label"
        ? "text-sm"
