...
export function LogoCCD({ taille = 36 }: { taille?: number }) {
  return (
    ...
    >
      <rect width="512" height="512" rx="114" fill="var(--color-primary-500)" />
...
export function MarqueCCD({
  ...
  return (
    <span className={cn("inline-flex min-w-0 items-center gap-3 no-underline", className)}>
      <LogoCCD taille={taille} />
      <span
        className={cn(
          "flex min-w-0 flex-col leading-tight",
          // Le signe seul sur telephone : c'est la place ainsi rendue qui
          // permet au logo du client de rester dans la barre.
          compacteSurMobile && "hidden md:flex",
        )}
      >
        <span className="text-lg font-bold tracking-[-0.02em] text-neutral-900">{t("nom")}</span>
        {/* 14 px est le plancher de lisibilite du Socle §7.3 ; il descend a
            13 px sur ecran large, et pas en dessous. */}
        <span className="text-sm font-medium whitespace-nowrap text-neutral-500 lg:text-[0.8125rem]">
          {t("accroche")}
        </span>
      </span>
    </span>
